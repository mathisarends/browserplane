...
Beide hängen an Infrastruktur, die es im Data Plane nicht gibt
...
(`data_plane/presentation/errors.py`).
...
```
data-plane/src/data_plane/features/browser_state/
  application/
...
### 8.1 Neue Settings (`DataPlaneSettings`)
...
- Jede `tab.url` muss Schema `http` oder `https` haben. Alles andere
  ...
  auf URLs im Data Plane, siehe 9.
...
| --- | --- |
| `SecurityPolicy.is_url_allowed` | Keine Allowlist im Data Plane. Der Worker führt aus, was das Control Plane ihm sagt; Policy gehört ins Control Plane. Es bleibt die Schema-Prüfung aus 8.2. |
| `EventBus` / `visualize=False` | Der Data Plane hat keinen Event-Bus für Navigation; der Screencast sieht die Änderungen ohnehin. |
| `BrowserTabs`-Registry mit Session-Cache | Der Data Plane hält keinen Tab-Zustand. Targets werden pro Operation frisch über `get_targets()` ermittelt, Sessions pro Operation attached und wieder detached. Zustand, den niemand invalidiert, geht sonst irgendwann schief. |
...
| `FocusedTabClient` | Es gibt keinen fokussierten Tab im Data Plane; alle Operationen sind browserweit. |
...
- Keine Cookie-Werte, localStorage- oder sessionStorage-Werte in Logs, auch
  ...
  und Fehlertypen.
- Kein Schreiben auf Platte im Data Plane. Der State existiert nur in der
  Response.
- Kein Caching der Response (`Cache-Control: no-store` auf dem GET).
- Der Data-Plane-Port ist nicht öffentlich. Beim späteren Durchreichen über
...
1. `data_plane/app.py`: `browser_state_router` in `ROUTERS`, die neuen Specs in
   `API_ERRORS`, `BrowserStateProvider()` in den Container.
2. `scripts/export_openapi_schemas.py` und `scripts/generate_http_clients.sh`
   laufen lassen → `generated/data_plane` bekommt `capture_browser_state` /
...
3. Control Plane: ein `browser_state`-Feature, das über den generierten Client
   ...
   Entscheidung, wo der State persistiert wird. Das Data Plane speichert
   bewusst nichts.

## 12. Tests

Gemäß `AGENTS.md` nur das Nötigste: `data-plane/tests/test_browser_state.py`
