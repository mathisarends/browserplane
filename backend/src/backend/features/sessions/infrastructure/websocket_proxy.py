...
async def proxy_stream(client: WebSocket, upstream_url: str, *, name: str) -> None:
    """Relay one frontend websocket to an internal plane, in both directions."""
    await client.accept()
    try:
        ...
        logger.warning("%s stream became unavailable (%s)", name, type(error).__name__)
        await _close_client(client, code=1011, reason=f"{name} unavailable")
    except Exception as error:
        # Upstream URLs may contain credentials, so don't log exception messages.
        logger.error(
            "%s websocket proxy stopped unexpectedly (%s)",
            name,
            type(error).__name__,
        )
        await _close_client(client, code=1011, reason=f"{name} unavailable")
