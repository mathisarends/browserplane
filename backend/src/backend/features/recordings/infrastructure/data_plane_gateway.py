...
from generated.data_plane import ApiError, GeneratedDataPlaneClient, RecordingResponse
...
class DataPlaneRecordingGateway(RecordingGateway):
    ...
    async def stop_and_store(
        ...
        try:
            async with AsyncClient(
                ...
                client = GeneratedDataPlaneClient(
                    http,
                    browser.slot.data_plane_url,
    ...
    async def _store_segment(
        ...
        url = f"{browser.slot.data_plane_url.rstrip('/')}{path}"
...
def _transfer_error(action: str, error: Exception) -> RecordingTransferException:
    logger.warning(
        "Could not %s data-plane recording: %s", action, type(error).__name__
    )
